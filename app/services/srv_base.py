...
class BaseService(Generic[ModelType], object):
    ...
    def create(self, data: dict[str, Any]) -> ModelType:
        ...
        # Oracle compatibility: Convert is_active bool to int
        if 'is_active' in obj_data and isinstance(obj_data['is_active'], bool):
            obj_data['is_active'] = 1 if obj_data['is_active'] else 0
        
        # Oracle compatibility: Extract roles for set_roles() method
        roles_data = obj_data.pop('roles', None)
        
        obj = self.model(**obj_data)
        
        # Oracle compatibility: Set roles using helper method
        if roles_data is not None and hasattr(obj, 'set_roles'):
            obj.set_roles(roles_data)
    ...
    def update_by_id(self, id: int, data: dict[str, Any]) -> ModelType:
        ...
        # Oracle compatibility: Handle roles separately
        roles_data = obj_data.pop('roles', None)
        ...
        # Oracle compatibility: Set roles using helper method
        if roles_data is not None and hasattr(exist_obj, 'set_roles'):
            exist_obj.set_roles(roles_data)
